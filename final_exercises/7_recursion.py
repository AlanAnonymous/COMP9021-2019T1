...
# Concise procedure. However, it sacrifices a certain amount of space complexity and time complexity
# not recommended
# "7_recursion_better.py" is better
def recursion(List_1, List_2, TheSum):
    if sum(List_1) == TheSum:
        Result_List.append((int(''.join(str(e) for e in List_1))))
    for i in range(len(List_2)):
        recursion(List_1 + [List_2[i]], List_2[i + 1:], TheSum)
