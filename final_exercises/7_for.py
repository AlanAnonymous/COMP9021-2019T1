...
4321, 12331, 12421, 13321]
    ...
    List_number, List = list(str(number)), ['']
    
    for i in range(len(List_number)):
        for j in range(len(List)):
            the_string = List[j] + List_number[i]
            if sum(int(s) for s in the_string) <= sum_of_digits:
                List.append(the_string)

    return sorted(set([int(string) for string in List if sum(int(s) for s in string) == sum_of_digits]))
